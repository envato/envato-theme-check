...
class Themeforest implements themecheck {
	...
	function check( $php_files, $css_files, $other_files )
	{
		...
		$req_checks = array(
			...
			'/(^|\s)(__|_e|_n|_x|_nx|_ex)\s*?\(/'           => __( 'All translatable strings must be escaped to prevent malicious code injection from a translation', 'theme-check' ),
		);
		...
		$warn_checks = array(
			...
			'/[^a-z0-9](?<!_)force_balance_tags\s?\( \$/'   => __( 'Possible data validation issues found. force_balance_tags() does not escape data', 'theme-check' ),
			'/(echo|print)\s*\(?\s*\$/'                     => __( 'Possible data validation issues found. All dynamic data must be correctly escaped for the context where it is rendered', 'theme-check' ),
